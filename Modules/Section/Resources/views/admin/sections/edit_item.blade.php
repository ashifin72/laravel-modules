...
@section('content')
    ...
    <section class="content card-body card">
        ...
        <form method="POST" action="{{route('site.admin.section_items.update', $item->id)}}">
            @method('PATCH')
            @csrf
            <div class="row">
                <div class="col-sm-8">
                    @forelse($locales as $locale)
                        ...
                        <div class="form-group">
                            ...
                            <textarea class="form-control" id="editor{{$loop->iteration}}" rows="9" name="content_{{$locale->local}}">
                                {{ $item->$content}}
                                    </textarea>
                        </div>
                    @empty
